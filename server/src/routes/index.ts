...
import { TextFile, type ITextFile } from '../models/TextFile.js';
...
router.delete("/api/topic/:id", adminCheckedToken, async(req: CustomRequest, res: Response) => {
    if (!req.user) {
...
router.post("/api/upload", validateToken, upload.single("image"), async (req: CustomRequest, res: Response) => {
     try {
        ...
        const textfile: ITextFile = new TextFile({
...
router.post("/api/file/:id/edit/start", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) {
        return res.status(401).json({ message: "Unauthorized" });
    }
...
router.delete("/api/delete/:id", validateToken,  async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
router.put("/api/file/:id/rename", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    try {
        const fileId = req.params.id;
        const file = await TextFile.findById(fileId);
...
router.get("/api/content/:id", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    try {
        const fileId = req.params.id;
        const file = await TextFile.findById(fileId);
...
router.put("/api/lock/:id", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    try {
        const fileId = req.params.id;
        const file = await TextFile.findById(fileId);
...
router.put("/api/unlock/:id", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    try {
        const fileId = req.params.id;
        const file = await TextFile.findById(fileId);
...
router.put("/api/read=true/:id", validateToken, async (req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({message: "Unauthorized"});
...
router.put("/api/read=false/:id", validateToken, async(req: CustomRequest, res: Response) => {
    if (!req.user) return res.status(401).json({message: "Unauthorized"});
    try {
        const fileId = req.params.id;
        const file = await TextFile.findById(fileId);
...
router.get("/api/file/nonAuthenticate", async (req: Request, res: Response) => {
    try {
...
router.get("/api/file/:id/downloadPDF", async (req: Request, res: Response)=> {
    try {
        ...
        res.setHeader(
        "Content-Disposition",
        `attachment; filename="${file.filename.replace(/\s+/g, "_")}.pdf"`
        );
...
router.post("/api/file/:id/comment", validateToken, async (req: CustomRequest, res: Response) => {
    try {
        if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
router.get("/api/file/:id/getComment", validateToken, async (req: CustomRequest, res: Response) => {
    try {
        if (!req.user) return res.status(401).json({message: "Unauthorized"});
        const file = await TextFile.findById(req.params.id);
